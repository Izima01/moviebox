...
const links = [
    ...
    { to: '/movies', name: 'Movies', logo: Movie },
    ...
    <aside className="border border-[rgba(0,0,0,0.3)] rounded-e-3xl py-6 w-48 lg:w-[14.5rem] flex flex-col justify-between items-center bg-white h-screen">
        ...
        <nav className='flex flex-col w-full'>
            {links.map(({to, name, logo}, i) => (
                <NavLink key={i} to={to} className={({isActive}) => `flex pl-6 py-4 text-lg w-full font-semibold ${isActive ? 'text-[#BE123C] bg-[#d6335c33] border-r-[#BE123C] border-r-4' : 'text-[#666]'}`}>
